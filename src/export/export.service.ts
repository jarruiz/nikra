...
export class ExportService {
  ...
  async exportParticipationsToExcel(filters: ExportQueryDto): Promise<Buffer> {
    ...
    worksheet.columns = [
      ...
      { header: 'Nombre Usuario', key: 'userNombre', width: 20 },
      { header: 'Apellidos Usuario', key: 'userApellidos', width: 25 },
    ...
    participations.forEach((participation, index) => {
      const row = worksheet.addRow({
        ...
        userNombre: participation.user?.nombre || '',
        userApellidos: participation.user?.apellidos || '',
  ...
  async exportParticipationsToCSV(filters: ExportQueryDto): Promise<string> {
    ...
    const csvRows = participations.map(participation => [
      ...
      participation.user?.nombre || '',
      participation.user?.apellidos || '',
